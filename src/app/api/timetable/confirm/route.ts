...
export async function POST(req: Request) {
  try {
    ...
    const classes = Array.from(new Set(rows.map((r) => r.className.trim()).filter(Boolean)));
    if (classes.length === 0) {
      return NextResponse.json({ ok: false, error: he.timetable.noRows }, { status: 400 });
    }

    // Replace is intentionally destructive per class. Guarding branchId + trimmed
    // required fields above prevents wiping the wrong school or writing blanks.
