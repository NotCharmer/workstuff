...
export async function POST(req: Request) {
  try {
    ...
    if (isTimetableCsv(file)) {
      ...
      return NextResponse.json({ ok: true, fileName: file.name, branchId, ...out.result });
    }
    ...
    return NextResponse.json({ ok: true, fileName: file.name, branchId, ...result });
  } catch (error) {
    if (error instanceof AuthError) {
      return NextResponse.json({ ok: false, error: error.message }, { status: error.status });
    }
    console.error("[timetable/parse]", error);
